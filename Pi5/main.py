...
from src.core.config import DEFAULT_MODEL, DEFAULT_PROFILE
from src.pipeline.ingest import parse_transcript, get_turn_stats, format_transcript_for_llm
...
from src.pipeline.extract import run_multi_pass_pipeline
...
def run_pipeline(transcript_text: str, model: str, profile: str, prepass_model: Optional[str] = None) -> None:
    """
    ...
    try:
        png_path, json_path = export_day_to_qr(dedup_output, Path("data/qrcodes"))
        print(f"\n  QR export saved: {png_path}")
    except Exception as exc:
        logger.warning("QR export failed (non-fatal): %s", exc)
...
def main() -> None:
    ...
    parser.add_argument("--prepass-model", type=str, default=None, help="Optional model for pre-pass stage only")
    ...
    if args.audio:
        ...
        print("\n[ERROR] Please provide --audio or --transcript.")
        sys.exit(1)
    ...
    try:
        run_pipeline(transcript_text, model=model, profile=profile, prepass_model=args.prepass_model)
