"""
Pipeline Step 4 — Multi-pass focused extraction.

Input:  diarized transcript text (str)
Output: DedupOutput  (merged beliefs/goals/states/relationships)
        IdentityOutput (name, age, occupation, location)

Why multiple passes instead of one big LLM call?
  A single LLM call asked to extract everything at once tends to be shallow.
  Breaking it into focused passes — one per category — lets the model go deeper:
  the core_value pass only hunts for values, the long_term_goal pass only looks
  for goals, and so on. The dedup step then merges and cross-reasons across all
  of them, which produces richer insights than any single pass could.

The four steps this module orchestrates:
  1. run_pre_pass()          — read transcript, build extraction plan
  2. run_focused_extraction() — one LLM call per category in priority order
  3. run_dedup()             — merge, deduplicate, find cross-category links
  4. run_identity_extraction()— extract name/age/job/city (smart update only)

The original single-pass approach (one LLM call for everything) lives in
  src/pipeline/extract.py git history — it was simpler but less thorough.
"""
...
def run_pre_pass(diarized_text: str, model: str) -> PrePassOutput:
    ...
    for core_cat in CORE_EXTRACTION_CATEGORIES:
        if core_cat not in existing_cats:
            priority += 1
            # __import__ used here to avoid a circular import at module level
            # (schemas imports logger, logger imports config — fine at runtime)
            result.passes.append(
                __import__("src.core.schemas", fromlist=["PassDefinition"]).PassDefinition(
                    category=core_cat,
...
def run_multi_pass_pipeline(
    ...
    Args:
        diarized_text: Diarized transcript text
        model: Model for focused extraction, dedup, and identity phases
        existing_graph: Existing knowledge graph for context
        prepass_model: Optional override model for pre-pass stage only
    ...
    # Phase 1: Pre-pass
    pre_pass_model = prepass_model or model
    if prepass_model and prepass_model != model:
        logger.info("Using separate pre-pass model: %s (main model: %s)", prepass_model, model)
    pre_pass = run_pre_pass(diarized_text, pre_pass_model)
    raw_logs.append(f"=== PRE-PASS (model: {pre_pass_model}) ===\n{json.dumps(pre_pass.model_dump(), indent=2)}")
